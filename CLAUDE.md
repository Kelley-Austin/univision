...
If you can use flow, use flow. Salesforce APEX should only be used if all low code solutions are not options.

If you are instructed to use APEX, reevaluate if flow is the better option. If it is, ask the user why APEX is needed.

Low code/no code is more maintainable. We aim for maintanablity.